...
int inter_guild_calcinfo(struct guild *g)
{
	...
	// Set the max number of members, Guild Extension skill - currently adds 2 to max per skill lv.
	g->max_member = BASE_GUILD_SIZE + inter_guild->checkskill(g, GD_EXTENSION) * 2;
	if(g->max_member > MAX_GUILD)
	{
